#!/bin/bash
# v0.0.3
# Compile Python 3.3.0 from scratch 
...
pybuild_add() {
	...
	export PREFIX="/opt/python-3.3.0"
...
pybuild_xz() {
	dl_if_none xz-5.0.4.tar.gz http://tukaani.org/xz/xz-5.0.4.tar.gz
	tar -C $BUILDDIR -zxf $SRCDIR/xz*
	cd $BUILDDIR/xz*
	pybuild_add
	./configure --prefix=$PREFIX
	make
...
pybuild_python() {
	dl_if_none Python-3.3.0.tar.bz2 http://www.python.org/ftp/python/3.3.0/Python-3.3.0.tar.bz2
